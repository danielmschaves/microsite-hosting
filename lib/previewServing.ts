...
// ---------------------------------------------------------------------------
// Preview-deployment serving, shared by the path-based route
// (app/s/[slug]/preview/[deploymentId]/[[...path]]/route.ts) and the
// host-based route (app/mbhost/[[...path]]/route.ts, CD-20) — extracted for
// the same reason as lib/siteServing.ts. `publicPath` is always the
// browser-visible path (what the password form must POST back to, what a
// redirect must land on) — callers compute it themselves rather than this
// module inferring it from req.url, since a host-routed request's req.url
// reflects the internal /mbhost-rewritten path, not what the visitor sees.
// ---------------------------------------------------------------------------
...
/**
 * Serve one preview deployment. Gated on deployments.status='ready' AND
 * target='preview', PLUS the deployment's own access_mode (CD-15): "inherit"
 * (default) keeps the exact pre-CD-15 behavior — the site's own canViewSite
 * policy; "organization" requires workspace membership regardless of the
 * site's own visibility; "password"/"hybrid" require a signed cookie proving
 * a prior password check — the plaintext password is never stored anywhere
 * but the argon2id hash on the deployment row.
 */
export async function servePreview(
  req: Request,
  opts: { slug: string; deploymentId: string; path: string[]; publicPath: string },
): Promise<NextResponse> {
  const { slug, deploymentId, path, publicPath } = opts;
  ...
  if (deployment.access_mode === "organization" || deployment.access_mode === "hybrid") {
    if (!lower) {
      ...
      loginUrl.searchParams.set("callbackUrl", publicPath);
  ...
  if (deployment.access_mode === "password" || deployment.access_mode === "hybrid") {
    ...
    if (!cookieValue || !verifyPreviewAccess(cookieValue, deploymentId)) {
      return passwordForm({ action: publicPath });
    }
  }
...
/**
 * Handle the password form's POST. `cookiePath` scopes the resulting cookie
 * — the path-based route scopes it to `/s/{slug}/preview/{deploymentId}`
 * (many previews can share one browser); the host-based route just uses `/`
 * since the host itself already scopes the cookie to this one preview.
 */
export async function handlePreviewPasswordSubmit(
  req: Request,
  opts: { slug: string; deploymentId: string; publicPath: string; cookiePath: string },
): Promise<NextResponse> {
  const { deploymentId, publicPath, cookiePath } = opts;

  const found = await findSiteAndReadyPreview(opts.slug, deploymentId);
  if (!found) {
    return new NextResponse("Preview not found or not ready", { status: 404 });
  }
  ...
  if (deployment.access_mode !== "password" && deployment.access_mode !== "hybrid") {
    return passwordForm({ action: publicPath, error: "This preview is not password-protected." });
  }

  const form = await req.formData().catch(() => null);
  const password = form?.get("password");
  if (typeof password !== "string" || !password) {
    return passwordForm({ action: publicPath, error: "Enter the password." });
  }
  ...
  if (!ok) {
    return passwordForm({ action: publicPath, error: "Incorrect password." });
  }

  const res = NextResponse.redirect(new URL(publicPath, req.url), { status: 303 });
